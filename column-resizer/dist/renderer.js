...
class ColumnResizerExtension extends LensExtensions.Renderer.LensExtension {
  ...
  onActivate() {
    console.log("✓ Column Resizer Pro v4.6 - Better scroll sync");

    // Clear saved widths on page load (so refresh resets columns)
  ...
  applyColumnWidthsViaCSS(tableId) {
    ...
    // If we have custom widths, estimate total table width and set min-width
    if (customColumnCount > 0) {
      const tableHead = document.querySelector('.TableHead');
      if (tableHead) {
        const totalCells = tableHead.querySelectorAll('.TableCell').length;
        // Estimate: custom columns + default 100px for other columns
        const estimatedWidth = totalCustomWidth + ((totalCells - customColumnCount) * 100);
        css += `
          .TableHead, .TableRow {
            min-width: ${estimatedWidth}px !important;
          }
        `;
      }
    }
  ...
  injectStyles() {
    ...
    this.styleElement.textContent = `
      ...
      /* Table layout - top scrollbar only, synced header */
      .VirtualList {
  ...
  setupScrollSync() {
    ...
    this.scrollSyncInterval = setInterval(() => this.syncScrollContainers(), 300);
  }

  removeScrollSync() {
    if (this.scrollSyncInterval) {
  ...
  syncScrollContainers() {
    ...
    if (!this.scrollListenerAdded) {
      ...
      // Listen to ALL scroll events and find the one that scrolls horizontally
      document.addEventListener('scroll', (e) => {
        const target = e.target;
        if (target && target.scrollLeft !== undefined) {
          // Move header to match any horizontal scroll
          tableHead.style.transform = `translateX(-${target.scrollLeft}px)`;
        }
      }, true);

      // Also listen specifically on VirtualList
      const virtualList = document.querySelector('.VirtualList');
      if (virtualList) {
        virtualList.addEventListener('scroll', () => {
          tableHead.style.transform = `translateX(-${virtualList.scrollLeft}px)`;
        });
      }

      // And on the Table's parent
      const table = document.querySelector('.Table');
      if (table && table.parentElement) {
        table.parentElement.addEventListener('scroll', () => {
          tableHead.style.transform = `translateX(-${table.parentElement.scrollLeft}px)`;
        });
      }
    }
  }
